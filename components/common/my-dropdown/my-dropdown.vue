<template>
	<view class="my_dropdown" @click.stop.prevent="() => {}">
		<view :class="['my_dropdown__menu', borderBottom ? 'my_border-bottom' : '']" id="my_dropdown__menu"
			:style="{ 'height': `${height}rpx`, 'background-color': backgroundColor }">
			<view class="my_dropdown__menu_mian">
				<view class="my_dropdown__menu__left" :style="{ height: `${height}rpx` }">
					<slot name="left"></slot>
				</view>
				<view class="my_dropdown__menu__center">
					<view class="my_dropdown__menu__item" :style="{ height: `${height}rpx` }" v-for="(item, index) in menuList"
						:key="index" @click="handlerMenuClick(index, item.dropdownKey)">
						<view class="my_flex">
				...
				<view class="my_dropdown__menu__right" :style="{ height: `${height}rpx` }">
					<slot name="right"></slot>
				</view>
			</view>
		</view>

		<uni-transition :mode-class="['fade']" :styles="maskClass" :duration="duration" :show="active"
			@click="handlerMaskClick" />
		<uni-transition :mode-class="['fade']" :styles="transClass" :duration="duration" :show="active">
			<view class="my_dropdown__content__popup">
				<slot></slot>
			</view>
		</uni-transition>
	</view>
...
<script>
	 ...
	 * @example <my-dropdown></my-dropdown>
	 */
	export default {
		name: 'my-dropdown',
		props: {
		...
		methods: {
			...
			getContentHeight() {
				// #ifdef APP-NVUE
				uni.createSelectorQuery().in(this).select('#my_dropdown__menu').boundingClientRect().exec(rect => {
					const data = rect[0];
				...
				// #ifndef APP-NVUE || H5
				uni.createSelectorQuery().in(this).select('.my_dropdown__menu').boundingClientRect(rect => {
...
<style lang="scss" scoped>
	...
	.my_dropdown__menu__item__text {
		font-size: 28rpx;
		color: #606266;
	}

	.my_dropdown__menu__item__arrow {
		@include vue-flex(row);
		margin-left: 5rpx;
		margin-top: 3rpx;
