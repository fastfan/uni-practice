	...
	<view v-if="isVisible" class="poster_slider" @click="handleClickOutside">
		<view class="poster_slider_container">
			<swiper
				...
				next-margin="60rpx"
				:style="{ marginTop: statusBarHeight + 44 + 'px', height: midHeight }"
			>
		...
		<button class="poster_slider_btn" @click.native.stop="clickShareBtn({ type: 'wechatMoment' })">
			...
			<view class="btn_text">分享至朋友圈</view>
		</button>
		<view class="poster_slider_share">
			<button
				class="share_list"
				@click.native.stop="clickShareBtn(shareList[0])"
				open-type="share"
				:data-share="{
					title: '分享的标题',
			...
			>
				<image :src="shareList[0].icon" mode="widthFix"></image>
		...
		<poster-model
			ref="PosterModel"
			v-if="showPainter"
			:painterProps="posterList[currentIndex]"
			style="position: fixed; left: -999rpx"
		></poster-model>
...
export default {
	...
	computed: {
		midHeight() {
			const { winHeight, statusBarHeight } = getApp().globalData
			console.log(getApp())
