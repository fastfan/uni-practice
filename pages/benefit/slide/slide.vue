			...
			<swiper @change="slideOn" :current="tabsId" style="flex-grow: 1;height:calc(100vh - 50rpx);">
				<swiper-item v-for="(tab, indexs) in contentList" :key="indexs">
					...
					<view v-if="indexs === tabsId">
						<swiper class="swiper" @change="changefun" @animationfinish="animationfinishfun" :current="current"
							:circular="false" :vertical="true">
							<block v-if="PayVideo">
								<swiper-item v-for="(item,indexes) in PayVideo" :key="item.id">
									<view class="swiper-item uni-bg-black">
										<video v-if="indexes == current" class="video" :id="'id'+indexes" :src="item.part_url"
											:custom-cache="false" :controls="true" :show-progress="true" :initial-time="0"
											:show-loading="true" :show-mute-btn="true" :loop="true" :autoplay="true" :muted="false"
											:play-strategy="2" :object-fit="'contain'" :direction="90" :enable-progress-gesture="false"
											:show-center-play-btn="false" vslide-gesture-in-fullscreen :play-btn-position="'center'"
